...
public class ChatCompletionRequest {
    ...
    /**
     * 是否对返回结果数据做兼容。
     * 24年1月发布的GLM_3_5_TURBO、GLM_4模型，与之前的模型在返回结果上有差异。
     * 开启true可以做兼容。
     */
     ...
     * 24年01月发布的GLM_3_5_TURBO、GLM_4 模型时新增
     */
    @JsonProperty("do_sample")
    private Boolean doSample = true;

    /**
     * 使用同步调用时，此参数应当设置为 Fasle 或者省略。
     * 表示模型生成完所有内容后一次性返回所有内容。
     * 如果设置为 True，模型将通过标准 Event Stream ，
     * 逐块返回模型生成内容。Event Stream 结束时会返回一条data: [DONE]消息。
     * 24年1月发布的 GLM_3_5_TURBO、GLM_4 模型时新增
     */
    ...
    /**
     * 模型输出最大tokens
     * 24年01月发布的 GLM_3_5_TURBO、GLM_4 模型时新增
     */
     ...
     * 输入给模型的会话信息
     * 用户输入的内容：role=user
     * 携带历史的内容：role=assistant
     */
    private List<Prompt> prompt;

    /**
     * 智谱AI sse 固定参数 Incremental = true [增量返回]
     */
    private boolean incremental = true;

    /**
     * sseformat, 用户兼容解决sse增量模式okhttpsse
     * 拦截data：后面空格问题，[data: hello]。
     * 只在增量模式下使用sseFormat。
     */
    ...
    public static class Prompt {
        ...
        public static class PromptBuilder {
            ...
            public String toString() {
                return "ChatCompletionRequest.Prompt.PromptBuilder{" +
                        "role='" + role + '\'' +
                        ", content='" + content + '\'' +
                        '}';
            }
    ...
    public static class Tool {
        ...
        public static class Function {
            ...
            // parameter 字段需要传入一个 Json Schema 对象，以准确地定义函数所接受的参数。若调用函数时不需要传入参数，省略该参数即可
